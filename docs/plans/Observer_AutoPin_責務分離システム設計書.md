...
📍 基準位置: マーカー部分 / テキスト部分 [📌 判定結果表示]
「高優先度タスク...」の [自動判定結果] を基準
...
#### 4.2.2 クリック位置検出・視覚フィードバック仕様

##### 📌 ピン表示演出システム
```javascript
// テキスト要素クリック時の処理フロー
1. 要素クリック → event.offsetX/Y で相対位置取得
...
#### 4.2.3 UX設計思想
- **位置選択の廃止**: ユーザーの直感的なクリック位置をそのまま利用
- **視覚的な確証**: ピン演出により「ここが基準点」であることを明示
- **設定の簡素化**: 詳細設定は折りたたみで隠し、必要時のみ表示
- **統一されたフロー**: 全要素タイプで「この設定でBB配置開始」の統一ボタン

### 4.3 設計思想の変更点

#### 4.3.1 クリック位置基準システム採用の理由
1. **直感的操作**: ユーザーが「ここにピンを置きたい」と思った位置そのものを利用
2. **選択肢の完全廃止**: 9アンカー選択・位置選択メニューを全て廃止
3. **視覚的フィードバック**: ピン演出により基準位置を明確化
4. **作業時間短縮**: クリック→ピン演出→確認→完了の4ステップで完結
5. **学習コスト削減**: 「LT, TC, RT...」などの専門用語を完全排除

#### 4.3.2 新しい責務分担（クリック位置基準）
```
AutoPin選択UI:
- 基準要素の確定
- クリック位置の検出・記録
...
#### 4.4.3 期待される効果（クリック位置基準）
- **UI操作時間**: 従来の70%削減（クリック→演出→確認で完結）
- **学習コスト**: 専門用語完全廃止により90%削減
- **直感性向上**: 「ここにピンを置きたい」位置そのものを利用
- **視覚的確証**: ピン演出により基準位置の認識が100%明確
- **作業効率**: 選択肢に迷う時間の完全削除
