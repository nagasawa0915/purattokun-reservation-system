...
| 用途 | マニュアル | 説明 |
|------|-----------|------|
| **🎯 安定性最重視・黒枠解決** | [⭐ STABLE_SPINE_RENDERER_GUIDE.md](./STABLE_SPINE_RENDERER_GUIDE.md) | **最高推奨** - 黒枠問題完全解決・毎回確実動作 |
...
### ⭐ StableSpineRenderer を選ぶべき場合（最高推奨）
- ✅ **確実性最重視**: 毎回エラーなく動作させたい
- ✅ **黒枠問題解決**: 口周りの黒枠を根本的に解決したい
- ✅ **商用プロジェクト**: 安定動作が必須の重要な用途
- ✅ **モジュール化**: 再利用可能な形で実装したい
- ✅ **複数キャラクター**: 複数のSpineキャラクターを管理
- ✅ **AI問題回避**: セッションごとの実装ばらつきを避けたい
...
1. **安定性・確実性を最重視しますか？**
   - Yes → ⭐ **StableSpineRenderer** （最高推奨）
   - No → 下記2へ

2. **黒枠問題を経験したことがありますか？**
   - Yes → ⭐ **StableSpineRenderer** （根本解決）
   - No → 下記3へ
...
5. **主な用途は何ですか？**
   - 安定性重視・複数キャラクター → ⭐ StableSpineRenderer
   - ボタン・装飾・エフェクト → シンプル版
