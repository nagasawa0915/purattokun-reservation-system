...
### 3. デバッグシステム
- 右上固定のデバッグパネル（本番環境では削除推奨）
- リアルタイムでスクロール位置、アニメーション状況、**Spineステータス**を監視
- コンソールログによる詳細な動作追跡
...
- **背景統合**: z-index: 1で背景画像より前面、雲(z-index: 0)より前面に配置
- **レスポンシブ対応**: 絶対位置指定だが画面サイズに応じた調整可能
...
- **デバッグパネル**: リアルタイムでSpineステータス監視（初期化中→WebGL準備完了→アニメーション実行中）
- **プレースホルダーモード**: Spine未読み込み時も動作確認可能
