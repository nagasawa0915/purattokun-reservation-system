...
class StableSpineRenderer {
    constructor(config = {}) {
        ...
        this.config = {
            ...
            position: {
                ...
                scaleX: config.position?.scaleX ?? 0.55,
    ...
    validateRequiredConfig() {
        if (!this.config.character || typeof this.config.character !== 'string') {
            throw new Error('❌ StableSpineRenderer: character パラメータが必要です（文字列）');
        }
    ...
    async initializeWebGL() {
        ...
        this.log(`🔍 WebGL設定: ${JSON.stringify(this.STABLE_WEBGL_CONFIG)}`, 'info');
    }
    ...
    async checkSpineLibrary() {
        ...
        const requiredClasses = [
            ...
            'Skeleton', 'AnimationState', 'AnimationStateData', 'SceneRenderer'
        ];
        
        const missingClasses = requiredClasses.filter(className => !window.spine[className]);
        
        if (missingClasses.length > 0) {
            throw new Error(`Spine WebGLに必要なクラスが不足: ${missingClasses.join(', ')}`);
        }
        ...
        return new Promise((resolve, reject) => {
            const check = () => {
                if (this.assetManager.isLoadingComplete()) {
                    ...
                    reject(new Error('Asset loading failed: ' + this.assetManager.getErrors()));
                } else {
    ...
    playAnimation(animationName, loop = null) {
        ...
        try {
            ...
            this.log(`🎬 アニメーション再生: ${animationName} (ループ: ${loop})`, 'info');
            return true;
    ...
    playSequence(animations) {
        if (!this.animationState || !Array.isArray(animations) || animations.length === 0) {
    ...
    setTransform(x = null, y = null, scaleX = null, scaleY = null) {
        ...
        this.log(`📍 位置・スケール更新: (${this.skeleton.x}, ${this.skeleton.y}) scale: ${this.skeleton.scaleX}`, 'info');
        return true;
    }
     ...
     */
    static createForCharacter(characterName, canvasSelector = '#spine-canvas', options = {}) {
     ...
     * 複数キャラクター対応ファクトリー
     */
    static async createMultiple(characters, containerSelector = '.spine-container') {
        const renderers = [];
