...
class SpineCharacterManager {
     ...
     */
    async init() {
        // ローカルライブラリからの読み込み待機（Phase B: CDN依存排除）
        let attempts = 0;
        const maxAttempts = 50; // 5秒間待機（ローカルファイルのため短縮）
        
        log(LogLevel.INFO, 'initialization', 'Waiting for Spine WebGL local library to load...');
        
        while (typeof spine === 'undefined' && attempts < maxAttempts) {
            await new Promise(resolve => setTimeout(resolve, 100));
            attempts++;
            if (attempts % 10 === 0) {
                log(LogLevel.DEBUG, 'initialization', `Local library loading attempt ${attempts}/${maxAttempts}...`);
            }
        }
        
        if (typeof spine === 'undefined') {
            log(LogLevel.ERROR, 'initialization', 'Spine WebGL runtime not loaded from local library after 5 seconds');
            log(LogLevel.ERROR, 'initialization', 'Check if assets/js/libs/spine-webgl.js exists and loads properly');
            return false;
        }
        ...
        try {
            ...
            log(LogLevel.INFO, 'initialization', 'Spine WebGL 4.1.* initialized successfully from local library');
