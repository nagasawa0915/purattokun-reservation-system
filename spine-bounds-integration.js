...
function debugBoundsState() {
    ...
    if (typeof indexBoundsManager !== 'undefined' && indexBoundsManager) {
        ...
        ['purattokun', 'nezumi'].forEach(characterId => {
            ...
            const character = typeof spineCharacters !== 'undefined' ? spineCharacters[characterId] : null;
...
class IndexSkeletonBoundsManager {
    ...
    async initialize() {
        ...
        try {
            ...
            if (!this.boundsManager) {
                logBounds('ERROR', 'Failed to create SpineSkeletonBounds instance - null result');
                return false;
            }
            
            // 必要なメソッドの存在確認
            const requiredMethods = ['initializeCharacterBounds', 'checkBoundsHit', 'setDebugMode'];
            for (const method of requiredMethods) {
                if (typeof this.boundsManager[method] !== 'function') {
                    logBounds('ERROR', `Required method ${method} not found in SpineSkeletonBounds`);
                    logBounds('ERROR', 'Available methods:', Object.getOwnPropertyNames(this.boundsManager));
                    return false;
                }
            }
    ...
    integrateCharacter(characterId, characterData) {
        ...
        if (!this.boundsManager || !characterData.spine) {
            ...
            logBounds('WARN', `Cannot integrate ${characterId}: missing bounds manager or Spine data`);
            return false;
        }
        ...
        console.log(`🎭 [${characterId}] スケルトンデータ詳細:`, {
            ...
            slots: characterData.spine.skeleton ? characterData.spine.skeleton.slots.length : 0,
            skins: characterData.spine.skeleton ? characterData.spine.skeleton.data.skins.length : 0,
        ...
        if (characterData.spine.skeleton && characterData.spine.skeleton.slots) {
            ...
            characterData.spine.skeleton.slots.forEach((slot, index) => {
                if (slot.attachment) {
                    ...
                    logBounds('DEBUG', `${characterId} slot ${index} (${slot.data.name}):`, {
        ...
        const boundsInitialized = this.boundsManager.initializeCharacterBounds(characterId, spineCharacter);
        console.log(`📋 [${characterId}] SkeletonBounds初期化結果: ${boundsInitialized ? '✅成功' : '❌失敗'}`);
        ...
        if (boundsInitialized) {
            ...
            logBounds('INFO', `SkeletonBounds integrated for character: ${characterId}`);
            ...
            logBounds('ERROR', `Failed to initialize SkeletonBounds for: ${characterId}`);
            return false;
        }
    }
    ...
    async forceInitialize() {
        ...
        try {
            ...
            if (typeof SpineSkeletonBounds === 'undefined') {
                ...
                logBounds('INFO', 'Attempting to dynamically load spine-skeleton-bounds.js...');
                try {
                    ...
                    logBounds('ERROR', 'Failed to dynamically load spine-skeleton-bounds.js:', loadError);
                    return false;
                }
            }
    ...
    setupClickHandler(characterId, canvas) {
        if (!canvas) {
            logBounds('WARN', `No canvas found for ${characterId}, skipping click handler setup`);
            return;
        }
        ...
        const clickHandler = (event) => {
            ...
            if (hitResult) {
                ...
                logBounds('DEBUG', `No bounds hit for ${characterId} - executing default click`);
    ...
    executePartAnimation(characterId, hitResult) {
        ...
        if (animationName) {
            ...
            logBounds('INFO', `Part-specific animation played for ${characterId}: ${animationName} (part: ${partName})`);
        } else {
    ...
    executeDefaultClick(characterId) {
        ...
        if (canvas && canvas.originalClickHandler) {
            ...
            if (clickAnimation) {
                ...
                logBounds('INFO', `Fallback click animation played for ${characterId}: ${clickAnimation}`);
            }
        }
    }
    ...
    playAnimation(characterId, animationName, loop = false) {
        ...
        if (typeof spineCharacters !== 'undefined' && spineCharacters[characterId]) {
            ...
            if (character.animationState) {
                try {
                    ...
                    logBounds('DEBUG', `Animation ${animationName} set for ${characterId} (loop: ${loop})`);
                    ...
                    if (!loop) {
                        ...
                        if (returnAnimation && character.skeleton.data.findAnimation(returnAnimation)) {
                            character.animationState.addAnimation(0, returnAnimation, true, 0);
                            logBounds('DEBUG', `Return animation ${returnAnimation} set for ${characterId}`);
                        }
                    }
                } catch (error) {
                    logBounds('ERROR', `Failed to play animation ${animationName} for ${characterId}:`, error);
                    ...
                    try {
                        ...
                        if (fallbackAnimation && character.skeleton.data.findAnimation(fallbackAnimation)) {
                            character.animationState.setAnimation(0, fallbackAnimation, true);
                            logBounds('DEBUG', `Fallback animation ${fallbackAnimation} set for ${characterId}`);
                        }
                    } catch (fallbackError) {
                        logBounds('ERROR', `Fallback animation also failed for ${characterId}:`, fallbackError);
                    }
                }
            } else {
            ...
            logBounds('WARN', `Character ${characterId} not found in spineCharacters`);
        }
    }
    ...
    drawBoundingBox(overlay, boundingBox, characterId, index) {
        ...
        logBounds('DEBUG', `Drew bounding box "${boundingBox.name}" for ${characterId}:`, {
            bounds,
            position: { left: leftPercent, top: topPercent, width: widthPercent, height: heightPercent }
        });
    }
...
window.integrateBoundsForCharacter = (characterId, characterData) => {
    return window.indexBoundsManager.integrateCharacter(characterId, characterData);
};
...
window.testNezumiBounds = () => {
    ...
    if (window.indexBoundsManager && window.indexBoundsManager.boundsManager) {
        const boundsInfo = window.indexBoundsManager.boundsManager.characters.get('nezumi');
        if (boundsInfo) {
...
window.testAllBounds = () => {
    ...
    ['purattokun', 'nezumi'].forEach(characterId => {
        if (window.indexBoundsManager && window.indexBoundsManager.boundsManager) {
            const boundsInfo = window.indexBoundsManager.boundsManager.characters.get(characterId);
            results[characterId] = {
...
function checkBoundsSystemStatus() {
    ...
    if (window.boundsManager) {
        console.log('登録済みキャラクター数:', window.boundsManager.characters?.size || 0);
        if (window.boundsManager.characters?.size > 0) {
            console.log('登録済みキャラクターID:', Array.from(window.boundsManager.characters.keys()));
        }
    }
    ...
    if (window.indexBoundsManager) {
        console.log('統合システム状態:', {
            ...
            統合済みキャラクター数: Object.keys(window.indexBoundsManager.integratedCharacters || {}).length
        });
    }
}
...
window.debugIntegrationDetails = (characterId) => {
    ...
    console.log('📋 BoundsManager状態:', {
        ...
        characters_keys: boundsManager?.characters ? Array.from(boundsManager.characters.keys()) : []
    });
    ...
    if (boundsManager?.characters?.has(characterId)) {
        ...
        if (boundsInfo?.skeletonBounds) {
            try {
                ...
                console.error(`❌ [${characterId}] SkeletonBounds状態取得エラー:`, error);
            }
        }
...
window.debugFullSystemState = () => {
    ...
    console.log('📋 基本システム状態:', {
        ...
        SpineSkeletonBounds_class_available: typeof SpineSkeletonBounds !== 'undefined'
    });
    ...
    if (manager) {
        console.log('🔍 初期化状態の詳細:', {
            ...
            'boundsManager_methods': manager.boundsManager ? Object.getOwnPropertyNames(manager.boundsManager) : 'N/A',
    ...
    try {
        const debugResult = boundsManager?.toggleBoundsDebug ? boundsManager.toggleBoundsDebug() : false;
